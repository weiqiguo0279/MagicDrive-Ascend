...
class Engine:
    ...
    def allocate_buffers(self, shape_dict=None, device="npu"):
...
class BaseModel:
    def __init__(self, model, fp16=False, device="npu", max_batch_size=16, embedding_dim=768, text_maxlen=77):
        self.model = model
        self.name = "SD Model"
        self.fp16 = fp16
        self.device = device
        ...
        if force_engine_rebuild or not os.path.exists(engine_path):
            ...
            if force_engine_rebuild or not os.path.exists(onnx_opt_path):
                if force_engine_rebuild or not os.path.exists(onnx_path):
                    ...
                    with torch.inference_mode(), torch.autocast("npu"):
...
class UNet(BaseModel):
    def __init__(
        self, model, fp16=False, device="npu", max_batch_size=16, embedding_dim=768, text_maxlen=77, unet_dim=4
    ):
...
class TensorRTStableDiffusionInpaintPipeline(StableDiffusionInpaintPipeline):
    ...
    def __call__(
        ...
        with torch.inference_mode(), torch.autocast("npu"), trt.Runtime(TRT_LOGGER):
