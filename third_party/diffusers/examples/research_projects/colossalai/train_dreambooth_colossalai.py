...
def main(args):
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            torch_dtype = torch.float16 if get_current_device() == "npu" else torch.float32
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            optimizer.step()
            lr_scheduler.step()
            logger.info(f"max GPU_mem cost is {torch_npu.npu.max_memory_allocated()/2**20} MB", ranks=[0])
