...
class DiffusionPipeline(ConfigMixin):
    ...
    def to(
        ...
        # .to("npu") would raise an error if the pipeline is sequentially offloaded, so we raise our own to make it clearer
        pipeline_is_sequentially_offloaded = any(
            module_is_sequentially_offloaded(module) for _, module in self.components.items()
        )
        if pipeline_is_sequentially_offloaded and torch.device(torch_device).type == "npu":
            raise ValueError(
        ...
        if pipeline_is_offloaded and torch.device(torch_device).type == "npu":
            logger.warning(
