...
class StableDiffusionPipelineFastTests(PipelineLatentTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    @unittest.skipIf(not torch_npu.npu.is_available(), reason="xformers requires cuda")
    def test_stable_diffusion_attn_processors(self):
        # disable_full_determinism()
        device = "npu"  # ensure determinism for the device-dependent torch.Generator
        components = self.get_dummy_components()
        sd_pipe = StableDiffusionPipeline(**components)
        sd_pipe = sd_pipe.to(device)
        sd_pipe.set_progress_bar_config(disable=None)
        ...
        attn_processors = {k: v.to("npu") for k, v in attn_processors.items()}
...
class StableDiffusionPipelineSlowTests(unittest.TestCase):
    ...
    def test_stable_diffusion_attention_slicing(self):
        ...
        mem_bytes = torch_npu.npu.max_memory_allocated()
        torch_npu.npu.reset_peak_memory_stats()
    ...
    def test_stable_diffusion_pipeline_with_model_offloading(self):
        ...
        mem_bytes_offloaded = torch_npu.npu.max_memory_allocated()
        ...
        pipe.enable_attention_slicing()
        _ = pipe(**inputs)
        mem_bytes_slicing = torch_npu.npu.max_memory_allocated()
