...
class PipelineTesterMixin:
    ...
    @unittest.skipIf(torch_device != "npu", reason="float16 requires CUDA")
    ...
    @unittest.skipIf(torch_device != "npu", reason="CUDA and CPU are required to switch devices")
    def test_to_device(self):
        ...
        pipe.to("npu")
        model_devices = [component.device.type for component in components.values() if hasattr(component, "device")]
        self.assertTrue(all(device == "npu" for device in model_devices))

        output_cuda = pipe(**self.get_dummy_inputs("npu"))[0]
    ...
    @unittest.skipIf(
        torch_device != "npu" or not is_accelerate_available() or is_accelerate_version("<", "0.14.0"),
    ...
    @unittest.skipIf(
        torch_device != "npu" or not is_xformers_available(),
