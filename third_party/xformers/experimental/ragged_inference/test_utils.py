...
def gpu_capabilities_older_than_70() -> bool:
    ...
    if _gpu_is_old is None:
        for i in range(torch_npu.npu.device_count()):
            major, _ = torch_npu.npu.get_device_capability(f"cuda:{i}")
            if major < 7:
                _gpu_is_old = True
        if _gpu_is_old is None:
            _gpu_is_old = False
...
def bf16_support():
    ...
    return torch_npu.npu.is_available() and (
        "RTX" in torch_npu.npu.get_device_name() or "A100" in torch_npu.npu.get_device_name()
    )
