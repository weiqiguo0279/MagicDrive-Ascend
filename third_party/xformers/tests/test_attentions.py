...
DEVICES = (
    [torch.device("cpu")] if not torch_npu.npu.is_available() else [torch.device("cuda")]
)

BATCH = 2
SEQ = 128 if torch_npu.npu.is_available() else 36
MODEL = 128 if torch_npu.npu.is_available() else 16
...
@pytest.mark.skipif(not torch_npu.npu.is_available(), reason="requires a CUDA gpu")
def test_causal(
    attention_name: str,
    heads: int,
):
...
@pytest.mark.parametrize("attention_name", ATTENTION_REGISTRY.keys())
@pytest.mark.skipif(torch_npu.npu.is_available(), reason="CUDA gpu not supported yet")
