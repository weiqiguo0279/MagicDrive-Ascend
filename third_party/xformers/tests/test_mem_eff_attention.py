...
cuda_only = pytest.mark.skipif(not torch_npu.npu.is_available(), reason="requires CUDA")
compute_capability = (0, 0)
if torch_npu.npu.is_available():
    compute_capability = torch_npu.npu.get_device_capability("cuda")
sm75_or_better_only = pytest.mark.skipif(
    compute_capability < (7, 5), reason="requires sm75+"
)
_devices = ["cpu", "cuda"] if torch_npu.npu.is_available() else ["cpu"]
...
def test_cuda_streams(
    ...
    s_hipri = torch_npu.npu.Stream(priority=-1)
    s_lopri = torch_npu.npu.Stream(priority=0)
    query, key, value, attn_bias = create_tensors(
        *opFW_device_dtype_biasT_B_Mq_Mkv_H_K_Kv, fmt="BMHK"
    )
    torch_npu.npu.synchronize()
    with torch_npu.npu.stream(s_lopri):
        torch_npu.npu._sleep(100_000_000)  # wait 100m cycles
        query *= 2
    s_hipri.wait_stream(s_lopri)
    with torch_npu.npu.stream(s_hipri):
