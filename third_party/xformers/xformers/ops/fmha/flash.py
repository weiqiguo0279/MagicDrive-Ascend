...
class FwOp(AttentionFwOpBase):
    ...
    def not_supported_reasons(cls, d: Inputs) -> List[str]:
        ...
        if d.device.type == "cuda":
            device_capability = torch_npu.npu.get_device_capability(d.device)
    ...
    def apply(
        ...
        rng_state = torch_npu.npu.get_rng_state() if inp.p != 0.0 else None
...
class BwOp(AttentionBwOpBase):
    ...
    def apply(cls, ctx: Context, inp: Inputs, grad: torch.Tensor) -> Gradients:
        ...
        if inp.p != 0.0:
            ...
            cur_rng_state = torch_npu.npu.get_rng_state()
            torch_npu.npu.set_rng_state(ctx.rng_state)
        cls.OPERATOR(
        ...
        if cur_rng_state is not None:
            torch_npu.npu.set_rng_state(cur_rng_state)
