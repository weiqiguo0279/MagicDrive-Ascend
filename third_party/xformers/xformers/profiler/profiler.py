...
class MemSnapshotsProfiler:
    ...
    def _has_trace_plot(self) -> bool:
        return hasattr(torch_npu.npu._memory_viz, "trace_plot")
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if not self._has_trace_plot:
            ...
            return
        assert self.enabled
        snapshot = torch_npu.npu.memory._snapshot()
        torch_npu.npu.memory._record_memory_history(False)
        ...
        with open(filename, "w+") as fd:
            fd.write(
                torch_npu.npu._memory_viz.trace_plot(
...
class _Profiler:
    ...
    def _enter_module(self, name) -> None:
        ...
        torch_npu.npu.nvtx.range_push(name)
