...
class _OpInfo:
    ...
    ev_start: torch_npu.npu.Event = field(
        default_factory=lambda: torch_npu.npu.Event(enable_timing=True)
    )
    ev_end: torch_npu.npu.Event = field(
        default_factory=lambda: torch_npu.npu.Event(enable_timing=True)
    )
...
class DetectSlowOpsProfiler(DispatcherWithoutBrokenFuncs):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        torch_npu.npu.synchronize()  # Wait for the events to be recorded
