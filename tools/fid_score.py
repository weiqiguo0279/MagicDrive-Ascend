...
from magicdrive.misc.inception import InceptionV3
from perception.common.nuscenes_utils import sample_token_from_scene
...
def resize_to_ratio(img: Image, ratio):
    w, h = img.size
...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu',
    ...
    logging.info(f"Using trans: {transforms}")
    ...
    for batch in tqdm(dataloader, ncols=80):
        ...
        if first_batch:
            logging.info(f"size of batch: {batch.shape}")
            first_batch = False
...
def calculate_activation_statistics(files, model, batch_size=50, dims=2048,
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    logging.info(f"images: {len(files)}")
...
def compute_statistics_of_path(path, model, batch_size, dims, device,
    ...
    if path.endswith('.npz'):
        ...
        # change this line to take any subfolders
...
def get_all_val_sample_tokens(nusc):
    ...
    for scene in nusc.scene:
        ...
        for fi in range(frames_len):
            ...
            sample_token = sample['next']  # this should be None for last one
    return tokens


def calculate_fid_given_tokens(
        roots, tokens, sensors, batch_size, device, dims, nusc, num_workers=1,
        transforms=None):
    """Calculates the FID of two roots with tokens"""
    files = [[], []]
    all_tokens = get_all_val_sample_tokens(nusc)
    logging.info(f"Total sample token length: {len(all_tokens)}")
    for sample_token in tqdm(all_tokens, ncols=80):
    ...
    logging.info(f"get activation for {roots[0]}")
    logging.info(f"samples: {len(files[0]) / len(sensors)}")
    m1, s1 = calculate_activation_statistics(
        files[0], model, batch_size, dims, device, num_workers, transforms)

    logging.info(f"get activation for {roots[1]}")
    logging.info(f"samples: {len(files[1]) / len(sensors)}")
    m2, s2 = calculate_activation_statistics(
        files[1], model, batch_size, dims, device, num_workers, transforms)
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)
...
def parse_args():
    ...
    parser.add_argument('--device', type=str, default=None,
                        help='Device to use. Like npu, npu:0 or cpu')
...
def path_main():
    ...
    if args.save_stats:
        save_fid_stats(args.path, args.batch_size,
                       device, args.dims, num_workers)
        return
...
def hydra_main(cfg):
    ...
    OmegaConf.save(config=cfg, f=os.path.join(
        HydraConfig.get().runtime.output_dir, "run_config.yaml"))
    ...
    # we may sample some tokens rather than use whole set
    ...
    transforms = TF.Compose([
        ...
        lambda x: top_center_crop(x, target_size=cfg.dataset.image_size),
        TF.ToTensor(),
    ])
    fid_value = calculate_fid_given_tokens(
...
def main():
    if sys.argv[1] == "paths":
        sys.argv.pop(1)
        print(f"Running from paths with argv: {sys.argv[1:]}")
        path_main()
    elif sys.argv[1] == "cfg":
        sys.argv.pop(1)
        print(f"Running from hydra config with argv: {sys.argv[1:]}")
        hydra_main()
    else:
        raise ValueError("Your first argument can only be 'paths' or 'cfg'.")
